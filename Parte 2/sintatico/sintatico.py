...
dicionario_reservado = {
    'interesse': 'a',
    'espaço': 'b',  # De fato ela nao existe para o processamento
    'identificador': 'c',
    'num_int' : 'd',
    'felizes_para_sempre': 'e',
    'parenteses_inicial': 'f',
    'parenteses_final': 'g',
    'chaves_inicial': 'h',
    'chaves_final' : 'i',
    'capturando_sentimentos': 'j',
    'desejo': 'k',
    'op_rel': 'l',
    'op_arit': 'm',
    'PROGRAMA': 'n',
    ...
    'DECLARACAO': 'q',
    'ATRIBUICAO': 'r',
    'ESTRUTURA_DE_DECISAO': 's',
    'ESTRUTURA_DE_REPETICAO': 't',
    'ENTRADA_SAIDA': 'u',
    'TIPO': 'v',
    'EXPRESSAO': 'w',
    'TERMO': 'x',
...
def remove_da_pilha(pilha_analise, acao, tabela_gramatica, tabela_slr):
    ...
    # Modifica a lista temporaria da posição 2 para frente com os valores do dicionario
    # Por exemplo PROGRAMA -> INSTRUCOES, ficaria na lista [PROGRAMA, ->, o]
    
    for i in range(2, len(producao)):
        producao[i] = dicionario_reservado[producao[i]]
...
def sintatico(lista_tokens):
    ...
    # Adiciona no path o diretório do arquivo "/sintatico/tabelaSLR.xlsx'"
    path = path + '/sintatico/tabelaSLR.xlsx'
    
    # Carrega a tabela
    tabela_slr, tabela_gramatica = carrega_tabela(path)
